...
from pen_classification_train_only import create_model
...
def show_history(history_csv_path):
    mlp.rcParams["font.family"] = "Malgun Gothic" # 한글깨짐 방지
...
if __name__ == "__main__":
    ...
    # 학습했던 가중치를 적용한다.
    model.load_weights("./CNN/pen_classification/model/weights/weights.08-1.05.hdf5")
    
    # 학습 추이 시각화
    show_history("./CNN/pen_classification/model/history.csv")

    # 라벨값을 dict로 생성하여 결과 표출 시 사용한다.
    labels = {0:"만년필", 1:"샤프", 2:"연필"}

    # 검증파일 경로 지정
    data_path = glob('./CNN/pen_classification/pen_dataset/val/*/*.jpg')

...
train_dir = "D:/Intel/CNN/pen_classification/pen_dataset/train"
test_dir = "D:/Intel/CNN/pen_classification/pen_dataset/test"
...
train_datagen = ImageDataGenerator(  # train dataset 용 이미지 전처리 과정
    rescale=1.0 / 255.0,  # Normalize를 위해 255로 나누어 줌
...
test_datagen = ImageDataGenerator(  # test dataset 용 이미지 전처리 과정
    rescale=1.0 / 255.0  # train의 DataGenerator와 같은 크기로 rescale해야 함
)
...
def create_model():
    ...
    model.add(
        layers.Conv2D(32, (3, 3), activation="relu", input_shape=input_shape) # 1차 합성곱 연산 시행
    )
    model.add(
        layers.MaxPooling2D((2, 2)) # (2, 2)크기로 max pooling 시행, 이 때 stride는 지정하지 않으면 pooling size로 지정된다.
    )
    ...
    model.add(
        layers.Dense(num_class, activation="softmax") # 출력층의 노드 수는 3개이며, 각 데이터의 확률값으로 출력하기 위한 softmax(확률의 총합 = 1)
    )
...
def fit_model():
    ...
    mch = ModelCheckpoint(
        filepath="./CNN/pen_classification/model/weights/weights.{epoch:02d}-{val_loss:.2f}.hdf5",
        monitor="val_loss",
        save_best_only=True,
        mode="min",
    )
    ...
    logger = CSVLogger("./CNN/pen_classification/model/history.csv")
